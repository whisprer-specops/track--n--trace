    ...
    pub fn new(mapping: SourceMappingConfig, timeout: Duration) -> Result<Self, AdapterError> {
        ...
        if timeout.is_zero() {
            return Err(AdapterError::Validation(
                "http adapter timeout must be greater than zero".into(),
            ));
        }
        Ok(Self { mapping, timeout })
    }
}
...
impl SourceAdapter for HttpJsonAdapter {
    ...
    fn pull(
        ...
        let body = http_get_text(&source.endpoint, self.timeout)?;
        let payload: Value =
...
impl FeedPollAdapter {
    pub fn new(mapping: SourceMappingConfig, timeout: Duration) -> Result<Self, AdapterError> {
        ...
        if timeout.is_zero() {
            return Err(AdapterError::Validation(
                "feed adapter timeout must be greater than zero".into(),
            ));
        }
        Ok(Self { mapping, timeout })
    }
...
fn http_get_text(endpoint: &str, timeout: Duration) -> Result<String, AdapterError> {

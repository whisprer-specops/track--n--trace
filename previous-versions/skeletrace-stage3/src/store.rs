...
impl EngineStore {
    ...
    pub fn prune_retained_samples(&mut self, now: Timestamp) {
        ...
        self.retained_samples.retain(|(_, metric_id), samples| {
            ...
            // Stage 3 only keeps an in-memory hot history buffer here.
            // Longer warm retention belongs in the later disk-backed store,
            // so pruning against hot_duration is the correct boundary.
            let cutoff = now
